...
The `*-mainnet-tracoor-debug-structlogs.json` fixtures are public-chain examples extracted from Tracoor execution block trace exports. Keep extracted fixtures small, retain the source export URL in `fixtures/provenance.json`, and do not commit full block-trace downloads unless there is a specific review need.
